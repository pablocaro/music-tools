...
(async () => {
  ...
  // --- 4 · the preview glide -------------------------------------------------
  {
    ...
    // A bar of 4/4, and the default rhythm row is Quarters — the gentlest rung
    // of both rows is what a first run opens on.
    ok("a bar of the default rhythm", steps.length === 4, JSON.stringify(steps.map((n) => n.y)));
